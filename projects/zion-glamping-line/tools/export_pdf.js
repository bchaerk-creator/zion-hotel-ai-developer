...
(async () => {
  ...
  const input = path.resolve(process.argv[2] || path.join(root, 'ZION_CASULO_SAFARI_Caderno_Tecnico.html'));
  const output = path.resolve(process.argv[3] || path.join(root, 'ZION_CASULO_SAFARI_Caderno_Tecnico.pdf'));
