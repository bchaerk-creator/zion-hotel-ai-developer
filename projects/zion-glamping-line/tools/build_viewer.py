...
function buildCasulo() {
  ...
  if (D.bico) { // Bico: membrana em balanço sobre o deck + cumeeira, bordas, costela e tirantes
    const bm = indexedMesh(D.bico.mesh.vertices, D.bico.mesh.faces, M.membrane); bm.name = 'bico';
    tube(D.bico.ridge, 0.057, M.steel, false, 48, st); D.bico.edges.forEach(e => tube(e, 0.03, M.steel, false, 72, st));
    D.bico.ribs.forEach(r => tube(r, 0.024, M.steel, false, 48, st)); D.bico.ties.forEach(t => tube(t, 0.007, M.cable, false, 8, st)); }
...
window.setView = setView; window.camera = camera; window.controls = controls;
