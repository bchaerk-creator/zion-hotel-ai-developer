...
if [[ "$1" != "--no-render" ]]; then
  echo "== 3D + renders (inclui a variante cocoon_s)"; python3 build_viewer.py && node render.js
...
for f in ../06_LOTES/*.html ../07_VARIANTES/*.html ../08_MOBILIARIO/*.html; do node export_pdf.js "$f" "${f%.html}.pdf"; done
