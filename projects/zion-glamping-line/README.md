...
|---|---|---|---|
| **ZION CASULO** | Cabana biomórfica em casulo: superelipse assimétrica com Bico em balanço de 2,40 m sobre o deck (ponta a 4,42 m), Espinha de Luz na cumeeira, seis Janelas Olho em lente e mini cozinha acoplada | Piso 9,60 x 5,86 m · concha 12,15 x 6,00 x 4,42 m com o Bico | 45,6 m² internos + vestíbulo 2,4 m² = 48 m² + deck 29,9 m² = 78 m² |
...
- **`04_INTERIORES/ZC-INT-001_Projeto_Interiores_Casulo.pdf`** (`.html`, `tools/build_interiores.py`; pranchas em `cocoon/interiores/`, `tools/interiores_cocoon.py`) — projeto de interiores do Casulo com marcenaria acoplada: IN-01 planta de layout de interiores cotada (MA-01 a MA-08 fixos, MS-01 a MS-09 soltos), IN-02 paginação de pisos e acabamentos, IN-03/04 elevações internas A a D, IN-05 detalhes da marcenaria (mini cozinha com cooktop de indução 2 bocas, forno compacto, geladeira e air fryer; armário na curva; cabeceira com criados suspensos; bancada), IN-06 quadro de mobiliário; memorial, quadros e FF&E. Anexado como seção 25 do documento técnico.
- **`06_LOTES (inclui `<TAG>-ONE-001_Teaser_Cotacao` por modelo e `ZG-RFQ-002_Fornecedores_SC`)/`** — contratação por modelo em 4 lotes separados (Casulo ZC, Safari ZS, Lodge 38 ZL, Cápsula ZK): `<TAG>-LOT1-001` deck e infraestrutura · `<TAG>-LOT2-001` estrutura metálica (para o serralheiro: lista ferro a ferro, geometria dos arcos para a calandra, coordenadas do Bico, tolerâncias, embalagem) · `<TAG>-LOT3-001` lonas, revestimentos e vidros (quadro de painéis, padrões de corte, fixação nos ferros, quadro de esquadrias) · `<TAG>-LOT4-001` mobílias (marcenaria acoplada, FF&E, equipamentos); cada um com escopo, interfaces, regras de modularidade para montanha, sequência e RFQ. `ZG-MP-001_Master_Plan.pdf` une tudo (estratégia, cronograma, matriz de interfaces, responsabilidades, riscos + os 16 lotes anexados) e `ZG-ONE-001_One_Pager.pdf` resume a linha numa página. Gerados por `tools/build_lotes.py`.
- **`<modelo>/lona/`** — padrões de corte das lonas externas, forros e revestimentos (`tools/lona.py`): `LN-01` mapa dos painéis, `LN-11…` um padrão por prancha (contorno, malha 0,50 m, bordas por tipo, recortes, tabela de estações), `LN-90` fixação da lona nos ferros (keder, bolsas, clamps, harpão, cabos), `<TAG>-LON-001_padroes.dxf` e `_coordenadas.xlsx` para o confeccionista.
- **`07_VARIANTES/ZC-ARQ-002_Casulo_Sensorial.pdf`** (`tools/sensorial.py`, geometria `CocoonSensorial`, pranchas em `cocoon_s/desenhos/`, lona em `cocoon_s/lona/`, 3D e renders em `cocoon_s/`) — variante do Casulo sem sobrescrever o original: cinturão transparente nos vãos A2-A4 (cristal com zíper + tela + cortina de voile e blackout), cama king com dossel olhando a fachada e o Bico, lounge de chão sob o Bico, mini cozinha em L junto ao banho, closet à direita; coleção de mobiliário Boho / Bali (`ffe.LISTS["cocoon_s"]`); renders comparativos e impactos nos lotes.
- **`00_BRIEFING/REVISOES.md`** — registro de revisões (Bico, mini cozinha, logo, Janelas Olho recentradas nos vãos entre arcos: 0,95 x 0,80 m).
- **`05_VIDEO/ZC-VID-001_Prompts_Video.md`** — direção e prompts para vídeo do Casulo na natureza por IA (Veo 3, Sora, Runway Gen-4, Kling, Luma): roteiro de 6 cenas, descrição de referência do produto, fluxo imagem → vídeo a partir dos renders; `cocoon/renders/zion-casulo-orbita.webm` é a órbita 3D gerada por `tools/turntable.js`.
- **Marca**: símbolo e logotipo horizontal oficiais da Zion vetorizados em `tools/svgkit.py` (`zion_mark`, `zion_logo`, versões HTML e DXF), aplicados em carimbos, capas, DXF e renders.
...
- **`interno/referencias/`** e **`interno/ZION_REFERENCIAS_NOMASTRA_H23_H28.{html,pdf,xlsx}`** — cotações de fornecedor (NOMASTRA H23 e H28, mai/2025) usadas como referência de mercado da Zion Lodge 24 e da Zion Lodge 28, com análise interna de custo posto (câmbio, frete e tributos como premissas ajustáveis em `tools/referencias.py`). Os comparativos técnicos sem preços aparecem no catálogo (páginas do Lodge 24 e do Lodge 28) e na apresentação.
